...
import {validateLiveCompletion} from './live-session-flow.mjs';
import {liveFeishuVisible} from './live-feishu-participants.mjs';
...
export function runVisible(t,a){if(a?.channel==='feishu-live')return liveFeishuVisible(t,a);return !!(t.runtime&&a.enabled&&!a.configurationOnly&&flowAllowed(a,t.workflow)&&(t.workflow==='06'?t.runtime.participants.includes(a.user.number):a.department||(t.runtime.creative?.automatic||t.runtime.creative?.local||t.runtime.localBusiness)&&t.runtime.participants.includes(a.user.number)||t.center===a.user.center&&(a.canManage||t.runtime.participants.includes(a.user.number))));}
...
export class FlowRuntime{
 ...
 validateOwner(n,number,nodes){const person=this.people().find(p=>p.number===number),flow=n.id.match(/^W(\d{2})\./)?.[1];const liveDelegate=n.liveFeishuAssignment===true&&this.liveParticipants?.canOwn(number,n.id);if(flow&&Array.isArray(person?.modules)&&!liveDelegate)requireFact(flowAllowed({enabled:person.active,modules:person.modules},flow),(person.name||number)+' 尚未开放此业务模块，请选择已有权限的同事或先完成授权',403);if(n.managerRequired)requireFact(['manager','director'].includes(person?.role),'此决定节点须由具备原业务授权的主管或总监办理',403);if(['W06.S6.E3','W06.S6.E4'].includes(n.id)){const other=nodes.find(x=>x.id===(n.id==='W06.S6.E3'?'W06.S6.E4':'W06.S6.E3'));requireFact(!other||other.owner.number!==number,'绩效复核负责人须与原评议负责人分开',403);}}
 ...
 create(a,b,key,{prepareOnly=false,definition=null,liveExecution=false}={}){requireFact(!liveExecution||prepareOnly&&b.workflow==='04'&&definition?.moduleOrder?.length===0,'直播委派仅允许由正式班表创建',403);requireFact(b.workflow!=='07','请在创意工作台创建任务，系统会自动同步到流程引擎',409);requireFact(a.canManage&&flowAllowed(a,b.workflow),'当前账号不能发起此流程',403);requireFact(typeof key==='string'&&key.length>=8&&key.length<=128,'缺少防重复提交编号');
  ...
  for(const n of nodes){if(liveExecution&&n.id==='W04.S4.E1')n.liveFeishuAssignment=true;this.validateOwner(n,n.owner.number,nodes);}
 ...
 command(a,taskId,action,b,key,{verifiedEvidence=null,liveReceipt=null}={}){if(a?.channel==='feishu-live')requireFact(action==='complete'&&a.taskId===taskId&&a.nodeId===b.nodeId,'飞书参与人只能提交本人指定执行节点',403);if(liveReceipt)requireFact(a?.channel==='feishu-live'&&action==='complete'&&liveReceipt.key===key&&/^[a-f0-9]{64}$/.test(liveReceipt.hash),'无效的飞书办理回执',403);requireFact(typeof key==='string'&&key.length>=8&&key.length<=128,'缺少防重复操作编号');const h=fingerprint(b),k=[a.user.number,taskId,action,key].join(':');
  this.store.transaction(s=>{this.ensure(s);const t=s.tasks.find(t=>t.id===taskId&&runVisible(t,a));requireFact(t,'流程不存在或没有权限',404);const prior=s.flowDedupe[k];if(prior){requireFact(prior.hash===h,'重复操作内容不一致',409);return t;}
   ...
   if(['pause','resume','cancel'].includes(action)){
    ...
    if(action==='complete'){
     ...
     if(t.runtime.liveSession)n.liveFacts=structuredClone(b.liveFacts);
     if(liveReceipt)n.liveFeishuReceipt={...liveReceipt,attempt:n.attempt};
 ...
 checkAssignments(){const people=this.people();if(!people.length)return;const active=new Set(people.filter(p=>p.active).map(p=>p.number));const problems=t=>[...new Map([t.runtime.manager,...t.runtime.nodes.filter(n=>n.state!=='completed').map(n=>n.owner)].filter(p=>!active.has(p.number)||!this.canNotify(p.number)).map(p=>[p.number,p])).values()];
  const needs=t=>t.runtime?.state==='running'&&(this.assignmentChecksReady?.(t)??true)&&fingerprint(problems(t).map(p=>p.number))!==(t.runtime.assignmentIssue?.signature||fingerprint([]));if(!this.store.read().tasks.some(needs))return;
