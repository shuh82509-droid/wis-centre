...
import {currentNodeNotice,withinNextDaySendWindow} from './flow-notice-validity.mjs';
import {currentNextDayGroup,isNextDayNotice,LIVE_WAR_ROOM_RECIPIENT,needsOfficialLiveSource} from './live-next-day.mjs';
import {currentNextDayReleaseLease} from './live-next-day-release.mjs';
import {futureLivePersonalNotice,stageFutureLiveIntent,recordFutureLiveResponse,
  holdFutureLiveUnknown,verifyFutureLiveReadback,commitFutureLiveReadback} from './live-future-message-evidence.mjs';
...
function holdUncertainNextDay(n){
  if(!isNextDayNotice(n)||!n.unknown&&n.state!=='sending')return false;
  n.state='attention';n.unknown=true;
  n.error='次日通知此前发送结果不明，已停止自动重试；请人工核验并独立读回原消息';
  delete n.leaseId;delete n.leaseUntil;
  return true;
}
function dueForFlush(n,now){
  return n.state==='ready'&&(n.nextAt<=now||isNextDayNotice(n)&&n.unknown)||
...
export class FlowFeishu{
  constructor(store,{people,fetchImpl=fetch,clock=Date.now,env=process.env,verifyLiveNoticeSource=null,readNextDayPermit=()=>null,releaseId=null,bootId=null}={}){this.store=store;this.people=people;this.fetch=fetchImpl;this.clock=clock;this.verifyLiveNoticeSource=verifyLiveNoticeSource;this.readNextDayPermit=readNextDayPermit;this.releaseId=releaseId;this.bootId=bootId;this.enabled=env.FLOW_NOTIFICATIONS_ENABLED==='true';this.futureEvidenceEnabled=env.FLOW_LIVE_FUTURE_EVIDENCE_ENABLED==='true';this.appId=env.FEISHU_APP_ID||'';this.secret=env.FEISHU_APP_SECRET||'';this.receiveType=env.FEISHU_RECEIVE_ID_TYPE||'open_id';this.warRoomChatId=env.FLOW_LIVE_WAR_ROOM_CHAT_ID==='oc_3f92ef62d6160399ee823e74def199e6'?env.FLOW_LIVE_WAR_ROOM_CHAT_ID:null;try{this.map=JSON.parse(env.FEISHU_RECIPIENT_MAP_JSON||'{}');}catch{this.map={};}this.base=env.FLOW_PUBLIC_URL||'https://app.fandow.top/fd-026222/wis-marketing-hub/workflow-panorama/';this.includeTaskId=env.FLOW_NOTIFICATION_INCLUDE_TASK_ID!=='false';this.token=null;this.flushing=false;}
 ...
 message(n,t){const node=t.runtime.nodes.find(x=>x.id===n.nodeId);const labels={ready:'有一项工作待你办理',returned:'工作已退回，请补充修改',overdue:'节点已超时，请处理或申请延期',escalated:'超时事项需要你协调',completed:'流程已完成',pause:'流程已暂停',resume:'流程已恢复',cancel:'流程已终止',source_attention:'自动流程需要处理',handoff_linked:'已有任务新增了上游交接，请核对',handoff_blocked:'流程交接需要你协调',assignment_attention:'流程负责人需要调整',routing_attention:'条件分支需要你协调'};
  ...
  async verifyFutureReadback(noticeId){
    const row=this.store.read().flowNotifications?.find(n=>n.id===noticeId);
    if(row?.state!=='verifying'||row.futureEvidence?.phase!=='verifying'||
      row.nextVerifyAt>this.clock())return;
    try{
      const token=await this.tenantToken(),receipt=row.futureEvidence;
      const headers={authorization:'Bearer '+token};
      const messageResponse=await this.fetch('https://open.feishu.cn/open-apis/im/v1/messages/mget?card_msg_content_type=raw_card_content&message_ids='+encodeURIComponent(receipt.messageId),
        {method:'GET',headers,redirect:'error',signal:AbortSignal.timeout(10000)});
      const messageBody=await messageResponse.json();
      requireFact(messageResponse.ok&&messageBody.code===0,'机器人消息回查暂不可用',503);
      const items=messageBody.data?.items||messageBody.data?.messages||[];
      const message=items.find(item=>item.message_id===receipt.messageId);
      requireFact(message,'机器人尚未读到原始消息',503);
      const chatResponse=await this.fetch('https://open.feishu.cn/open-apis/im/v1/chats/'+encodeURIComponent(receipt.chatId),
        {method:'GET',headers,redirect:'error',signal:AbortSignal.timeout(10000)});
      const chatBody=await chatResponse.json();
      requireFact(chatResponse.ok&&chatBody.code===0,'机器人会话回查暂不可用',503);
      const verified=verifyFutureLiveReadback(row,message,chatBody.data,this.appId);
      commitFutureLiveReadback(this.store,noticeId,verified,this.clock);
    }catch(error){
      this.store.transaction(s=>{const current=s.flowNotifications?.find(n=>n.id===noticeId);
        if(current?.state!=='verifying'||current.futureEvidence?.phase!=='verifying')return;
        current.nextVerifyAt=this.clock()+60000;
        current.error=error?.status===409?'机器人消息回查内容不符，需人工核验；不会重发':'机器人只读回查尚未完成；不会重发';
        if(error?.status===409){current.state='attention';current.unknown=true;}
      });
    }
  }
      ...
      prepared=stageFutureLiveIntent(this.store,{noticeId:notice.id,leaseId:notice.leaseId,
        ...
        validNotice:(row,s,task,node)=>currentNodeNotice(row,task,node,this.clock())&&
          ...
          (!isNextDayNotice(row)||currentNextDayReleaseLease(this.readNextDayPermit(),row,this.clock(),
            {releaseId:this.releaseId,bootId:this.bootId})&&withinNextDaySendWindow(this.clock()))});
    }catch(error){
      this.store.transaction(s=>{const row=s.flowNotifications?.find(n=>n.id===notice.id);
        if(row?.leaseId!==notice.leaseId)return;
        row.state='attention';row.error='本人通知发送前身份、场次或租约未通过核验；未发送';
  ...
  async flush(){if(this.flushing||!this.enabled||!this.appId||!this.secret)return;this.flushing=true;try{await this.verifyPendingFutureReadbacks();for(let i=0;i<5;i++){
   ...
   let state='ready',messageId=null,error='',unknown=false,futureHandled=false;try{const recipient=this.recipient(n.recipient);if(!recipient){state='attention';error='未找到唯一且在职的飞书收件人映射';}else{
     ...
     if([99991663,99991668].includes(d.code)&&refreshAttempt++===0){this.token=null;continue;}if(r.ok&&d.code===0&&d.data?.message_id){state='sent';messageId=d.data.message_id;}else if(r.ok&&d.code===0){state='attention';unknown=true;error='飞书返回成功但未给出消息 ID，发送结果不明；请独立读回原消息';}else if([429,500,502,503,504].includes(r.status)){unknown=r.status>=500;error='飞书暂时繁忙，原事件将退避重试';}else{state='attention';error='飞书发送失败，错误码 '+String(d.code??r.status);if(d.code===99991663||d.code===99991668)this.token=null;}break;}
    }}catch{unknown=true;error='发送结果暂不确定，将用同一消息编号核验重试';}
    if(futureHandled)continue;
    this.store.transaction(s=>{const row=s.flowNotifications.find(x=>x.id===n.id);if(row?.leaseId!==n.leaseId)return null;row.unknown=state==='sent'?false:Boolean(row.unknown||unknown);const uncertainNextDay=isNextDayNotice(row)&&row.unknown;row.state=uncertainNextDay?'attention':state==='ready'&&row.attempts>=5?'attention':state;row.error=uncertainNextDay?'次日通知发送结果不明，已停止自动重试；请人工核验并独立读回原消息':error;row.messageId=messageId||row.messageId;if(state==='sent')row.sentAt=new Date(this.clock()).toISOString();row.nextAt=this.clock()+Math.min(120000,5000*2**row.attempts);delete row.leaseId;delete row.leaseUntil;return true;});
  }}finally{this.flushing=false;}}
  retry(a,id){return this.store.transaction(s=>{const n=s.flowNotifications?.find(n=>n.id===id);const t=s.tasks.find(t=>t.id===n?.taskId);requireFact(n&&t&&t.runtime?.participants.includes(a.user.number)&&a.canManage,'没有该通知的处理权限',403);requireFact(n.state==='attention','只允许重试待处理通知',409);requireFact(!n.futureEvidence,'该本人消息已有发送意图，只能核验原消息，禁止重发',409);requireFact(!isNextDayNotice(n)||!n.unknown,'次日通知发送结果不明，必须先人工核验并独立读回原消息；禁止重发',409);requireFact(!n.unknown||this.clock()-Date.parse(n.firstAttemptAt||n.createdAt)<3600000,'消息结果不明且超出幂等核验窗口，请人工核验，已停止重发',409);n.state='ready';n.attempts=0;n.nextAt=this.clock();return {id:n.id,state:n.state};});}
}
