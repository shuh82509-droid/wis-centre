...
function fixture(t){
 ...
 let changed=false;
 const raw=date=>({date,updatedAt:new Date(now).toISOString(),writebackCapability:{enabled:true},rooms:[{code:'test',name:'测试',anchors:[[changed?'10:00':'09:00','12:00','主播']],assistants:[['09:00','12:00','助理']]}],sourceStatus:{test:{found:true,revision:4,sheetId:'sheet'}}});
 ...
 return {auto,live,store,people,advance:()=>now+=301000,change:()=>changed=true};
}
test('后台依正式来源派工，重复轮询不重复任务或通知',async t=>{
 const f=fixture(t);await f.auto.tick();assert.equal(f.store.read().tasks.length,1);assert.equal(f.store.read().flowNotifications.length,1);
 f.advance();await f.auto.tick();assert.equal(f.store.read().tasks.length,1);assert.equal(f.store.read().flowNotifications.length,1);
