...
type Slot={key:string;signature:string;date:string;roomName:string;startAt:string;endAt:string;anchor:string;assistants:string[];anchorName?:string;assistantNames?:string[];roomLead?:{number:string;name:string};sourceIssue?:string;cancelledTasks?:{id:string;version:number;title:string;sessionKey:string;taskUrl:string}[]};
...
export function LiveDailyWork({session,compact=false}:{session:HubSession;compact?:boolean}){
  ...
  const scope=JSON.stringify([session.user.number,session.workspace.role,session.workspace.center,session.access.allowed_modules,session.access.policy_state]);
  if(!enabled)return null;
  return <LiveWork key={scope} compact={compact} calendarAdmin={!!(session.permissions?.super_admin||session.permissions?.manage_permissions)}/>;
}
function LiveWork({compact,calendarAdmin}:{compact:boolean;calendarAdmin:boolean}){
  ...
  const alive=useRef(true),epoch=useRef(0),locked=useRef(false);
  async function refresh(preserveError=false){const version=++epoch.current;try{const next=await request('live/today');if(alive.current&&version===epoch.current){setData(next);if(initialRead.current){initialRead.current=false;setExpanded(next.items.some((i:Item)=>i.state==='running'&&i.node.state==='ready'&&!i.session.sourceIssue));}if(!preserveError)setError('');setDate(v=>v||next.date);}}catch(e){if(alive.current&&version===epoch.current){setData(null);setChosen(null);setError(e instanceof Error?e.message:'读取失败');}}}
  ...
  async function submit(s:Submission){
    ...
    catch(e){if(!alive.current)return;const status=(e as {status?:number}).status;if(!status||status>=500){setUncertain(s);setError('提交结果尚不确定。请先读回原任务；继续同一次提交会沿用原编号，避免重复办理。');}else{setUncertain(null);setError(e instanceof Error?e.message:'未完成');}await refresh(true);}
  ...
  const content=<>
    <header><div><h2>直播今日工作</h2><p>备播与排班 → 话术与素材准备 → 上播准备 → 直播执行 → 复盘与行动</p></div><button type="button" disabled={busy} onClick={()=>void refresh()}>刷新我的工作</button></header>
    ...
    {data&&<><p className="live-work-meta">{data.date} · 最近读取 {when(data.generatedAt)}{!data.enabled?' · 新场次派工待启用；未启用不代表当天无工作。':''}</p>
      {data.items.length===0?<p>当前尚无已派发给你的直播工作。请以正式班表为准，不能据此认定今天无需上班。</p>:<ul>{data.items.map(item=><li key={item.taskId+item.node.id}><div><strong>{item.node.liveStage} · {item.node.title}</strong><p>{item.title} · 截止 {when(item.node.dueAt||item.node.plannedDueAt)}</p><small>{item.session.sourceIssue|| (item.state==='paused'?'任务已暂停':item.node.state==='ready'?'轮到我办理':'等待：'+item.waitingFor.join('、'))}</small></div><div><a href={item.taskUrl}>查看原任务</a><button disabled={busy||!!uncertain||!!item.session.sourceIssue||item.state!=='running'||item.node.state!=='ready'} onClick={()=>setChosen(item)}>办理</button></div></li>)}</ul>}
