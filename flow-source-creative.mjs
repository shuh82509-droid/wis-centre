  ...
  try{
   ...
   for(const row of data.records){
    try{
     ...
     const watch=mirrored.creativeWatches?.find(w=>w.source==='idea'&&w.recordId===row.id);
     ...
     // The notification sender requires a persisted verification <45 s old.
     // Unchanged mirrors keep that heartbeat at 20 s while each 5 s poll
     // still checks the current projection, owners and action ledger.
     if(watch?.automatic&&watch.taskId===task?.id&&!watch.issue&&JSON.stringify(watch.manager)===JSON.stringify(manager)&&
   ...
   for(const w of r.store.read().creativeWatches||[])if(w.source==='idea'&&w.automatic)this.creative.fail(w,Object.assign(Error(this.connection.issue),{status:503}));
