...
export class ProductionCreativeBridge{
 ...
 async sync(){
  ...
  try{
   ...
   for(const id of watchIds.filter(id=>!data.records.some(d=>d.id===id))){const error=Object.assign(Error('原任务已删除或暂不可读，保留最后核验状态'),{status:409});this.creative.fail({source:'idea',recordId:id},error);issues.push({recordId:id,message:error.message});}
