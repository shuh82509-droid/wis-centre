...
export class SourceBusinessBridge{
 ...
 apply(source,row,{prior:knownPrior,known=false,forceVerify=false}={}){const r=this.runtime,d=definitions[source];
  ...
  // The notifier needs a persisted verification younger than 45 seconds.
  // Rechecked, unchanged rows can use the existing heartbeat for 20 seconds.
  ...
  if(!forceVerify&&local?.production&&local.source===source&&local.recordId===row.id&&local.signature===signature&&!local.issue&&age>=0&&age<20000)return prior;
 ...
 sync(snapshot){for(const unused of this.syncSteps(snapshot)){} }
 async syncCooperatively(snapshot,{forceVerify=false}={}){for(const unused of this.syncSteps(snapshot,{forceVerify}))await yieldToEventLoop();}
 *syncSteps(snapshot,{forceVerify=false}={}){
  ...
  for(const source of ['remix','cloud']){
   ...
   for(const row of rows){yield;
    // Cooperative sync yields between rows, so a prebuilt known-task map
    // can become stale. Re-read this row immediately before applying it.
    const sourceKey='source:'+source+':'+row.id,current=r.store.read().tasks.find(t=>t.runtime?.sourceKey===sourceKey);
    if(baseline.has(row.id)&&!current)continue;
    try{this.apply(source,row,{prior:current,known:true,forceVerify});}
    catch(e){const issue={recordId:row.id,message:e.status?e.message:'来源任务暂未完成核验'};issues.push(issue);
     const latest=r.store.read().tasks.find(t=>t.runtime?.sourceKey===sourceKey);
     if(latest&&latest.runtime.localBusiness?.issue!==issue.message)r.store.transaction(s=>{const t=s.tasks.find(t=>t.runtime?.sourceKey===sourceKey);if(t)t.runtime.localBusiness.issue=issue.message;return true;});}
   }
   const missing='原记录暂不可读，保留最后核验状态',present=new Set(rows.map(row=>row.id));
   if(r.store.read().tasks.some(t=>t.runtime?.localBusiness?.production&&t.runtime.localBusiness.source===source&&!present.has(t.runtime.localBusiness.recordId)&&t.runtime.localBusiness.issue!==missing))
    r.store.transaction(s=>{for(const t of s.tasks.filter(t=>t.runtime?.localBusiness?.production&&t.runtime.localBusiness.source===source&&!present.has(t.runtime.localBusiness.recordId)))t.runtime.localBusiness.issue=missing;return true;});
