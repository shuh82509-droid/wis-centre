...
export class SourceBusinessBridge{
 ...
 apply(source,row){const r=this.runtime,d=definitions[source];
 ...
 sync(snapshot){
  ...
  for(const source of ['remix','cloud']){
   ...
   for(const row of rows){if(baseline.has(row.id)&&!r.store.read().tasks.some(t=>t.runtime?.sourceKey==='source:'+source+':'+row.id))continue;try{this.apply(source,row);}catch(e){issues.push({recordId:row.id,message:e.status?e.message:'来源任务暂未完成核验'});r.store.transaction(s=>{const t=s.tasks.find(t=>t.runtime?.sourceKey==='source:'+source+':'+row.id);if(t)t.runtime.localBusiness.issue=issues.at(-1).message;return true;});}}
   r.store.transaction(s=>{for(const t of s.tasks.filter(t=>t.runtime?.localBusiness?.production&&t.runtime.localBusiness.source===source))if(!rows.some(x=>x.id===t.runtime.localBusiness.recordId))t.runtime.localBusiness.issue='原记录暂不可读，保留最后核验状态';return true;});
  ...
  r.store.transaction(s=>{for(const child of s.tasks.filter(t=>t.runtime?.localBusiness?.production&&t.runtime.localBusiness.upstreamRenderId)){
